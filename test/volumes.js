...
tests["l toBest imperial"] = () => {
  var expectedVal = 1.33666;
  var expectedUnit = "ft3";
  var actual = convert(37.85).from("l").toBest({ toSystem: "imperial" });

  assert.ok(
    percentError(expectedVal, actual.val) < ACCURACY,
    "Expected: " + expectedVal + ", Actual: " + actual
  );
  assert.ok(
    expectedUnit === actual.unit,
    "Expected: " + expectedVal + ", Actual: " + actual
  );
};

tests["gal toBest metric"] = () => {
  var expectedVal = 37.85;
  var expectedUnit = "l";
